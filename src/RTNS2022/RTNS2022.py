...
def schedule_link(link, scheduled_frame, link_to_flow, next_link, prev_link,
    ...
    for i in link_to_flow[link]:
        ...
        if next_link_i == None:
            offset = task_attr[i]['deadline'] - task_attr[i][
                't_trans'] - net_attr[link]['t_proc']
        else:
            offset = min(
                scheduled_frame[next_link_i][i][1] - task_attr[i]['t_trans'] -
                net_attr[next_link_i]['t_proc'], task_attr[i]['deadline'] -
                task_attr[i]['t_trans'] - net_attr[next_link_i]['t_proc'])
...
def RTNS2022(task_path, net_path, piid, config_path="./", workers=1):
    try:
        ...
        for link in scheduled_frame:
            for flow in scheduled_frame[link]:
                ...
                for ins in range(0, LCM // task_attr[flow]['period']):
                    start = scheduled_frame[link][flow][
                        1] + ins * task_attr[flow]['period']
        ...
        for link in scheduled_frame:
            for flow in scheduled_frame[link]:
                if link == task_attr[flow]['s_route'][0]:
                    ...
                    OFFSET.append([
                        flow, 0,
                        (task_attr[flow]['period'] - start) * utils.t_slot
                    ])
        ...
        for i in task_attr:
            ...
            DELAY.append([
                i, 0, (end_time - start_time + task_attr[i]['t_trans']) *
                utils.t_slot
            ])

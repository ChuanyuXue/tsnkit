...
def collision(link, flow, offset, scheduled_frame, task_attr):
    ...
    for flow_j, sche in same_link_frames.items():
        ...
        lcm = np.lcm(p_i, p_j)
        for u, v in [
            (u, v) for u in range(0, int(lcm / p_i)) for v in range(0, int(lcm / p_j))
        ]:
...
def get_potential_order_violate_set(
    link, flow, offset, scheduled_frame, task_attr, next_link, pre_link
) -> list:
    violate_set = []
            ...
            for flow_j in scheduled_frame[next_link_j]:
                if flow_j != flow:
                    ...
                    if q_j == q_i:
                        ...
                        violate_set.append([flow, flow_j, o_i, on_i, o_j, on_j])
    return violate_set
...
def schedule_link(
    link, scheduled_frame, link_to_flow, next_link, prev_link, task_attr, net_attr
):
    ...
    for i in link_to_flow[link]:
        ...
        if next_link_i == None:
            ...
            offset = min(
                scheduled_frame[next_link_i][i][1]
                - task_attr[i]["t_trans"]
        ...
        while flag:
            ...
            collision_set = get_potential_order_violate_set(
                link, i, offset, scheduled_frame, task_attr, next_link, prev_link
            )
...
def RTNS2022(task_path, net_path, piid, config_path="./", workers=1):
    try:
        ...
        for k in task_attr:
            task_attr[k]["q"] = 0
            task_attr[k]["priority"] = (
                len(task_attr[k]["s_route"])
                * task_attr[k]["t_trans"]
                / task_attr[k]["period"]
            )
        ...
        for link in scheduled_frame:
            for flow in scheduled_frame[link]:
                ...
                for ins in range(0, LCM // task_attr[flow]["period"]):
                    start = (
                        scheduled_frame[link][flow][1] + ins * task_attr[flow]["period"]
                    )
        ...
        for i in task_attr:
            ...
            DELAY.append(
                [i, 0, (end_time - start_time + task_attr[i]["t_trans"]) * utils.t_slot]
            )

        utils.write_result(
            utils.myname(), piid, GCL, OFFSET, ROUTE, QUEUE, DELAY, config_path
        )
...
if __name__ == "__main__":
    DATA = "../../testbed/stream.csv"
    TOPO = "../../testbed/topo.csv"
    RTNS2022(DATA, TOPO, 0)

...
class cp_wa:
    ...
    def solve(self) -> utils.Statistics:
        ...
        return utils.Statistics("-", result, solve_time, 0, solve_time,
    ...
    def add_frame_const(self):
        for s in self.task:
            for l in s.links:
                for k in s.get_frame_indexes(self.task.lcm):
                    self.solver.add(
                        self.solver.start_at_start(self.phi[s][l][k],
                                                   self.phi[s][l][k + 1],
                                                   s.period))
    ...
    def add_flow_trans_const(self):
        for s in self.task:
            for l in s.links[:-1]:
                ...
                self.solver.add(
                    self.solver.end_before_start(self.phi[s][l][0],
                                                 self.phi[s][next_l][0],
                                                 l.t_proc))
        ...
        for s in self.task:
            for l in s.links:
                _start = self.model_output.get_value(  # type: ignore
                    self.phi[s][l][0]).start
                _end = self.model_output.get_value(  # type: ignore
                    self.phi[s][l][0]).end
                _queue = self.model_output.get_value(  # type: ignore
                    self.p[s][l])
                for k in s.get_frame_indexes(self.task.lcm):
                    gcl.append([
                        l, _queue, _start + k * s.period, _end + k * s.period,
                        self.task.lcm
                    ])
    ...
    def get_offset(self) -> utils.Release:
        ...
        for s in self.task:
            _release = self.model_output.get_value(  # type: ignore
                self.phi[s][s.first_link][0]).start

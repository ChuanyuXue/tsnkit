...
class i_ilp:
    ...
    def routing(self):
        ...
        for s1, s2 in self.task.get_pairs():
            doc_net[s1][s2] = doc_net[s2][s1] = \
                len(set(flat_paths(paths[s1])) & set(flat_paths(paths[s2]))) * \
                    s1.t_trans_1g * s2.t_trans_1g / (s1.period * s2.period)
        ...
        costs = [sum([
            len(set(paths[s1][opt[s1]].links) & set(paths[s2][opt[s2]].links)) * \
                s1.t_trans_1g * s2.t_trans_1g / (s1.period * s2.period)
                for s2 in self.task if s1 != s2]) for s1 in self.task]
        ...
        for _ in range(self.iter):
            ...
            for m in range(len(paths[i])):
                if m != opt[i]:
                    cost = sum([len(set(paths[i][m].links) & set(paths[j][opt[j]].links)) * \
                    i.t_trans_1g * j.t_trans_1g / i.period * j.period for j in self.task])
                    if cost < best:
                        best = cost
                        m_star = m
            opt[i] = m_star
    ...
    def solve(self) -> utils.Statistics:
        ...
        for epoch in range(max(self.task_group) + 1):
            ...
            self.t = self.solver.addMVar(shape=len(self.task),
            ...
            if self.solver.status == 3:
                return utils.Statistics("-", utils.Result.unschedulable,
                                        total_time)
            elif self.solver.status in {9, 10, 11, 12, 16, 17}:
                return utils.Statistics("-", utils.Result.unknown, total_time,
                                        total_time)
        ...
        for s in self.task:
            ...
            for l in s.links:
                if l == s.first_link:
                    ...
                    self.delay[s][l] = self.delay[s][
                        _prev_hop] + l.t_proc + s.get_t_trans(l)

    def add_frame_const(self, epoch) -> None:
        for s in self.task:
            if self.task_group[s] != epoch:
                continue
            self.solver.addConstr(0 <= self.t[s], )
            self.solver.addConstr(
                self.t[s] <= s.period - self.delay[s][s.last_link])
                ...
                if self.task_group[s2] == epoch:
                    ...
                    for k1, k2 in self.task.get_frame_index_pairs(s1, s2):
                        _temp = self.solver.addVar(
                            vtype=gp.GRB.BINARY,
                            name=f"{l}_{s1}_{s2}_{k1}_{k2}")
                        self.solver.addConstr(
                            self.t[s2] + k2 * s2.period - self.t[s1] +
                            k1 * s1.period - self.delay[s1][l] +
                            s1.get_t_trans(l) +
                            self.delay[s2][l] <= utils.T_M * _temp)
                        self.solver.addConstr(
                            self.t[s1] + k1 * s1.period - self.t[s2] +
                            k2 * s2.period - self.delay[s2][l] +
                            s2.get_t_trans(l) +
                            self.delay[s1][l] <= utils.T_M * (1 - _temp))
                    ...
                    for k1, k2 in self.task.get_frame_index_pairs(s1, s2):
                        _temp = self.solver.addVar(
                            ...
                            name=f"{l}_{s1}_{s2}_{k1}_{k2}")
                        self.solver.addConstr(
                            self.solution[s2] + k2 * s2.period - self.t[s1] +
                            k1 * s1.period - self.delay[s1][l] +
                            s1.get_t_trans(l) +
                            self.delay[s2][l] <= utils.T_M * _temp)
                        self.solver.addConstr(
                            self.t[s1] + k1 * s1.period - self.solution[s2] +
                            k2 * s2.period - self.delay[s2][l] +
                            s2.get_t_trans(l) +
                            self.delay[s1][l] <= utils.T_M * (1 - _temp))
    ...
    def get_delay(self) -> utils.Delay:
        delay = []
        for s in self.task:
            delay.append([s, 0, self.delay[s][s.last_link]])

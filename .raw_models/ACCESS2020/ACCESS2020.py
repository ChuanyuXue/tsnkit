...
def ACCESS2020(DATA, TOPO, NUM_FLOW, INS=-1, OUTPUT="./", workers=1, delta=14):
    try:
        (
            net,
            net_attr,
            ...
            sw_set,
            rate,
        ) = utils.read_network(TOPO, utils.t_slot)
        ...
        for i in task_attr:
            ...
            task_attr[i]["rho"] = (
                (int(task_attr[i]["t_trans"] + utils.delta))
                * (len(route) - 1)
                / task_attr[i]["deadline"]
            )
            for j in range(int(LCM / task_attr[i]["period"])):
                packet_weight[(i, j)] = (
                    task_attr[i]["deadline"] + j * task_attr[i]["period"]
                )
            for _i, a in enumerate(route[:-1]):
                ...
                for j in range(int(LCM / task_attr[i]["period"])):
                    task_var[i][link]["alpha"].append(
                        z3.Int("alpha" + str(i) + "_" + str(link) + "_" + str(j))
                    )
        ...
        for f, attr in sorted(
            task_attr.items(), key=lambda x: x[1]["rho"], reverse=True
        ):
            min_h = -1
            min_value = np.inf
        ...
        for interation in range(NG):
                ...
                for link in task_var[i]:
                    for k, ins in enumerate(task_var[i][link]["alpha"]):
                        if task_var[i][link]["group"][k] == interation:
                            s.add(
                                k * (task_attr[i]["period"]) <= ins,
                                ins <= (k + 1) * (task_attr[i]["period"]),
                            )
            ...
            for i in task_var:
                ...
                for hop, link in enumerate(links[:-1]):
                    for k, ins in enumerate(task_var[i][link]["alpha"]):
                        if task_var[i][link]["group"][k] == interation:
                            s.add(
                                ins + task_attr[i]["t_trans"] + net_attr[link]["t_proc"]
                                <= task_var[i][links[hop + 1]]["alpha"][k]
                            )
                ...
                for k, ins in enumerate(task_var[i][e_hop]["alpha"]):
                    if task_var[i][s_hop]["group"][k] == interation:
                        s.add(
                            ...
                            <= task_var[i][s_hop]["alpha"][k] + task_attr[i]["deadline"]
                        )
            ...
            for link in net_var:
                for fa, fg in [
                    ...
                    for fa in task_attr
                    for fg in task_attr
                    if fg > fa and link in task_var[fa] and link in task_var[fg]
                ]:
                    for k, m in [
                        (k, m)
                        for k in range(len(task_var[fa][link]["alpha"]))
                        for m in range(len(task_var[fg][link]["alpha"]))
                    ]:
                        if (
                            task_var[fa][link]["group"][k] == interation
                            and task_var[fg][link]["group"][m] == interation
                        ):
            ...
            for fa, fg in [(fa, fg) for fa in task_attr for fg in task_attr if fg > fa]:
                ...
                for x_a, y_a, a_b in [
                    (path_i[_x - 1], path_j[_y - 1], i_a_b)
                    for _x, i_a_b in enumerate(path_i)
                    for _y, j_a_b in enumerate(path_j)
                    if i_a_b == j_a_b
                    and task_var[fa][i_a_b]["q"] == task_var[fa][j_a_b]["q"]
                ]:
                    for k, m in [
                        (k, m)
                        for k in range(len(task_var[fa][x_a]["alpha"]))
                        for m in range(len(task_var[fg][y_a]["alpha"]))
                    ]:
                        if (
                            task_var[fa][x_a]["group"][k] == interation
                            and task_var[fg][y_a]["group"][m] == interation
                        ):
                            s.add(
                                z3.Or(
                                    ...
                                    [
                                        ...
                                        z3.And(
                                            ...
                                            z3.Or(
                                                net_var[link]["t"][x]
                                                == task_var[i][link]["alpha"][ins],
                                                task_var[i][last_link]["alpha"][ins]
                                                + task_attr[i]["t_trans"]
            ...
            if res == z3.unsat:
                return utils.rprint(INS, NUM_FLOW, "infeasible", total_time)
            elif res == z3.unknown:
                return utils.rprint(INS, NUM_FLOW, "unknown", total_time)
            result = s.model()
            ...
            for link in net_var:
                ...
                for x in range(NUM_WINDOW - 1):
                    if (
                        result[net_var[link]["t"][x + 1]].as_long()
                        - result[net_var[link]["t"][x]].as_long()
                        > 0
                    ):
        ...
        for i, result in enumerate(group_result):
            for link in net_var:
                for t in range(NUM_WINDOW - 1):
                    if (
                        result[net_var[link]["t"][t + 1]].as_long()
                        - result[net_var[link]["t"][t]].as_long()
                        > 0
                    ):
            ...
            for ins_id in range(LCM // task_attr[i]["period"]):
                ...
                for interation, result in enumerate(group_result):
                    if task_var[i][start_link]["group"][ins_id] == interation:
                        start = result[
                            task_var[i][start_link]["alpha"][ins_id]
                        ].as_long()
                end_link = list(task_var[i].keys())[-1]
                for interation, result in enumerate(group_result):
                    if task_var[i][end_link]["group"][ins_id] == interation:
                        end = result[task_var[i][end_link]["alpha"][ins_id]].as_long()
                delay = (end - start + task_attr[i]["t_trans"]) * utils.t_slot
                DELAY.append([i, ins_id, delay])

        utils.write_result(
            utils.myname(), DATA, GCL, OFFSET, ROUTE, QUEUE, DELAY, OUTPUT
        )

        return utils.rprint(INS, NUM_FLOW, "sat", total_time)
...
if __name__ == "__main__":
    ...
    ACCESS2020(DATA, TOPO, var, ins, workers=4)

...
def CIE2021(DATA, TOPO, NUM_FLOW, INS=-1, OUTPUT="./", workers=1):
    try:
        ...
        for link in net_attr:
            no_overlap_links = [
                task_var[i][link_i]["phi"]
                for i in task_var
                for link_i in task_var[i]
                if link_i == link
            ]
        ...
        for i in task_var:
            ...
            s.add(
                ...
                - s.start_of(task_var[i][in_link]["phi"][0])
                ...
                for a, b in [
                    ...
                    s.add(
                        s.if_then(
                            ...
                            s.logical_or(
                                s.start_of(task_var[i][a_b]["phi"][0]) + a * i_period
                                < s.start_of(task_var[j][y_a]["phi"][0])
                                + b * j_period
                                + net_attr[a_b]["t_proc"],
                                s.start_of(task_var[j][a_b]["phi"][0]) + b * j_period
                ...
                for k in range(int(LCM / t)):
                    GCL.append(
                        [
                            eval(e),
                            queue,
                            (start + k * t) * utils.t_slot,
                            (end + k * t) * utils.t_slot,
                            LCM * utils.t_slot,
                        ]
                    )
        ## Offset
        ...
        for i in task_var:
            ...
            OFFSET.append([i, 0, (task_attr[i]["period"] - offset) * utils.t_slot])
